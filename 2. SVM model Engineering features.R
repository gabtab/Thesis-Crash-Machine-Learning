...
for (i in dfmag$TSTNO){
  ...
  results = datapoints(datap, ind.imp,ind.dat$Time,ind.dat$Force.X, ind.dat$Force.Y, ind.dat$Force.Z)
  results$TSTNO = i
...
if (datap == 5) {coldel = c(38,39)
  
}else if (datap == 10){coldel = c(23,24)
}else if (datap == 1){coldel = c(158,159)
}else if (datap == 50){coldel = c(11,12)}
...
summary(as.factor(alldataengineeringup$damrat))
...
alldatacarfeatures = NULL
datap = 5
if (datap == 5) {coldelf = c(46,47)

}else if (datap == 10){coldelf = c(31,32)
}else if (datap == 1){coldelf = c(166,167)
}else if (datap == 50){coldelf = c(19,20)}
...
alldatacarfeatures = alldatacarfeatures[,-coldelf]
set.seed(1001)
intrain <- createDataPartition(y = alldatacarfeatures$DamLev, p= 0.5, list = FALSE)
training = alldatacarfeatures[intrain,]
testing <- alldatacarfeatures[-intrain,]
